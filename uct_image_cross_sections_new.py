...
if draw_porosity_profile_image:
    ...
    df=pd.read_excel('scan_analysis_large.xlsx', sheet_name='Distros',
                         header=0,skiprows=[0])
...
if draw_cross_section_image: 
    ...
    #im_1A_cs1 = np.load('cross_section_images/filtered_1A_cs1.npy')
    #im_1A_cs2 = np.load('cross_section_images/filtered_1A_cs2.npy')
    
    im_1A_axrot30_cs1 = np.load('cross_section_images/filtered_1A_axrot30_cs1.npy')
    im_1A_axrot30_cs2 = np.load('cross_section_images/filtered_1A_axrot30_cs2.npy')
    
    im_3A_cs1 = np.load('cross_section_images/filtered_3A_cs1.npy')
    im_3A_cs2 = np.load('cross_section_images/filtered_3A_cs2.npy')
    
    im_5A_cs1 = np.load('cross_section_images/filtered_5A_cs1.npy')
    im_5A_cs2 = np.load('cross_section_images/filtered_5A_cs2.npy')
    
    im_6A_cs1 = np.load('cross_section_images/filtered_6A_cs1.npy')
    im_6A_cs2 = np.load('cross_section_images/filtered_6A_cs2.npy')
    
    im_7A_cs1 = np.load('cross_section_images/filtered_7A_cs1.npy')
    im_7A_cs2 = np.load('cross_section_images/filtered_7A_cs2.npy')
    ...
    #ax1.imshow(im_1A_cs2, vmin=vmin, vmax=vmax, cmap='gray', interpolation='nearest')
    #plt.axis('off')
...
if both_in_same:
    ...
    axt = fig.add_subplot(2,4,1)
    axt.plot(0.01*vpp_1A,0.1*np.linspace(0,len(vpp_1A)-1,len(vpp_1A)),ls[0],lw=lw,c=colors[0],label='1A')
    axt.plot(0.01*vpp_3A,0.1*np.linspace(0,len(vpp_3A)-1,len(vpp_3A)),ls[1],lw=lw,c=colors[1],label='3A')
    axt.plot(0.01*vpp_5A,0.1*np.linspace(0,len(vpp_5A)-1,len(vpp_5A)),ls[2],lw=lw,c=colors[2],label='5A')
    axt.plot(0.01*vpp_7A,0.1*np.linspace(0,len(vpp_7A)-1,len(vpp_7A)),ls[3],lw=lw,c=colors[3],label='7A')
    axt.legend(bbox_to_anchor=(0.650, 0.315), bbox_transform=axt.transAxes, handletextpad=0.55)
    axt.text(subtpos_porosity_t[0], subtpos_porosity_t[1], subt[4], transform=axt.transAxes) 
    axt.text(0.700, 0.685, 'Sample', transform=axt.transAxes)  
    axt.set_xlim([0.00, 0.260])
    axt.set_ylim([0.0, 86])
    axt.invert_yaxis()
    ...
    axb = fig.add_subplot(2,4,3)
    axb.plot(radii,hpp_1A[~hpp_1A.isna()],ls[0],lw=lw,c=colors[0],label='1A')
    axb.plot(radii,hpp_3A[~hpp_3A.isna()],ls[1],lw=lw,c=colors[1],label='3A')
    axb.plot(radii,hpp_5A[~hpp_5A.isna()],ls[2],lw=lw,c=colors[2],label='5A')
    axb.plot(radii,hpp_7A[~hpp_7A.isna()],ls[3],lw=lw,c=colors[3],label='7A')
    axb.text(subtpos_porosity_b[0], subtpos_porosity_b[1], subt[5], transform=axb.transAxes)
    axb.set_xlim([0, 47])
    axb.set_ylim([-0.02, 0.52])
    ...
    axb.xaxis.set_label_coords(0.5, -0.18) #-0.14
    
    axt.xaxis.set_label_coords(0.5, -0.08)
    
    #plt.legend()
    
    axt.set_position([0.690, 0.425, 0.25, 0.51])
    axb.set_position([0.690, 0.095, 0.25, 0.215])
    ...
    ax1.set_position([-0.030, 0.53, 0.40, 0.40])
    ax2.set_position([0.260, 0.53, 0.40, 0.40])
    ax3.set_position([-0.030, 0.06, 0.40, 0.40])
    ax4.set_position([0.260, 0.06, 0.40, 0.40])

    #plt.savefig('uCT_images.pdf',dpi=300)
